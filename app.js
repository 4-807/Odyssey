...
function confirmPlayer(socket, socketID) {
    console.log("confirm player enter the room: " + socketID);
...
io.on('connection', function (socket) {
    ...
    socket.on('new-player-confirm', function (socketID) {
        var ret = confirmPlayer(socket, socketID);
        if(ret) {
    ...
    socket.on('quit-player', function (socketID) {
        ...
        var room_id = quitPlayer(socketID);
        if(room_id != -1) {
            io.to('room-' + room_id).emit('quit-notify', socketID);
        }
    });
